...
function __munmap_js(addr, len, prot, flags, fd, offset) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(12, 1, addr, len, prot, flags, fd, offset);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
  if (prot & 2) {
...
function _bn_GF2m_mul_2x2() {
 err("missing function: bn_GF2m_mul_2x2");
 abort(-1);
}

function _bn_gather5() {
 err("missing function: bn_gather5");
 abort(-1);
}

function _bn_get_bits5() {
 err("missing function: bn_get_bits5");
 abort(-1);
}

function _bn_mul_mont_gather5() {
 err("missing function: bn_mul_mont_gather5");
 abort(-1);
}

function _bn_power5() {
 err("missing function: bn_power5");
 abort(-1);
}

function _bn_scatter5() {
 err("missing function: bn_scatter5");
 abort(-1);
}
